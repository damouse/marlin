...
#define BED_HYSTERESIS \
    2  // (°C) Only set the relevant heater state when ABS(T-target) > BED_HYSTERESIS
#endif
#endif
...
#define CHAMBER_FAN_MODE \
    2  // Fan control mode: 0=Static; 1=Linear increase when temp is higher than target; 2=V-shaped
       // curve.
#if CHAMBER_FAN_MODE == 0
#define CHAMBER_FAN_BASE 255  // Chamber fan PWM (0-255)
#elif CHAMBER_FAN_MODE == 1
#define CHAMBER_FAN_BASE \
    128  // Base chamber fan PWM (0-255); turns on when chamber temperature is above the target
...
#define HIGH_EXCESS_HEAT_LIMIT \
    5  // How much above target temp to consider there is excess heat in the chamber
...
#define HOMING_BUMP_MM \
    { 10, 10, 2 }  // (mm) Backoff from endstops after first bump
 ...
 * function If the machine cannot raise the probe fast enough after a trigger, it may enter a fault
 * state.
 */
...
#define SDSORT_USES_STACK \
    false  // Prefer the stack for pre-sorting to give back some SRAM. (Negated by next 2 options.)
#define SDSORT_CACHE_NAMES \
    true  // Keep sorted items in RAM longer for speedy performance. Most expensive option.
#define SDSORT_DYNAMIC_RAM \
    false  // Use dynamic allocation (within SD menus). Least expensive option. Set SDSORT_LIMIT
           // before use!
 ...
 * jerk) print acceleration will be reduced during the affected moves to keep within the limit.
 *
...
#define RETRACT_RECOVER_LENGTH \
    0  // (mm) Default additional recover length (added to retract length on recover)
#define RETRACT_RECOVER_LENGTH_SWAP \
    0  // (mm) Default additional swap recover length (added to retract length on recover from
       // toolchange)
...
#define FILAMENT_CHANGE_FAST_LOAD_FEEDRATE \
    8  // (mm/s) Load filament feedrate. This can be pretty fast.
...
#define PAUSE_PARK_NOZZLE_TIMEOUT \
    120  // (seconds) Time limit before the nozzle is turned off for safety.
#define FILAMENT_CHANGE_ALERT_BEEPS 6  // Number of alert beeps to play when a response is needed.
#define PAUSE_PARK_NO_STEPPER_TIMEOUT  // Enable for XYZ steppers to stay powered on during filament
                                       // change.
 ...
 * Relevant G-codes:
 * M906 - Set or get motor current in milliamps using axis codes X, Y, Z, E. Report values if no
 * axis codes given. M911 - Report stepper driver overtemperature pre-warn condition. M912 - Clear
 * stepper driver overtemperature pre-warn condition flag. M122 - Report driver parameters (Requires
 * TMC_DEBUG)
 */
...
#define X_STALLCURRENT \
    1500  // (mA) Current where the driver detects a stall (VALID: 31.25 * (1-128) -  4A max -
          // rounds down)
 ...
 * Other detected conditions can be used to stop the current print.
 * Relevant G-codes:
 * M906 - I1/2/3/4/5  Set or get motor drive level using axis codes X, Y, Z, E. Report values if no
 * axis codes given. I not present or I0 or I1 - X, Y, Z or E0 I2 - X2, Y2, Z2 or E1 I3 - Z3 or E3
 *         I4 - Z4 or E4
 *         I5 - E5
...
#define SPINDLE_LASER_PWM_INVERT \
    false  // Set to "true" if the speed/power goes up when you want it to go slower
 ...
 *
 * LASER_POWER_INLINE_TRAPEZOID_CONT_PER defines how many step cycles there are between power
 * updates. If your board isn't able to generate steps fast enough (and you are using
 * LASER_POWER_INLINE_TRAPEZOID_CONT), increase this. Note that when this is zero it means it occurs
 * every cycle; 1 means a delay wait one cycle then run, etc.
 */
...
#define FILWIDTH_ERROR_MARGIN      \
    (DEFAULT_STDDEV_FILAMENT_DIA * \
     4)  // (mm) If a measurement differs too much from nominal width ignore it
#define MAX_MEASUREMENT_DELAY \
    20  // (bytes) Buffer size for stored measurements (1 byte per cm). Must be larger than
...
#define POWER_MONITOR_CURRENT_OFFSET \
    -1  // Offset value for current sensors with linear function output
...
#define POWER_MONITOR_FIXED_VOLTAGE \
    13.6  // Voltage for a current sensor with no voltage sensor (for power display)
#endif
...
// Use a rolling average to identify persistant errors that indicate skips, as opposed to vibration
// and noise.
 ...
 * "Z_move_comp" string to enable synchronization with DLP projector exposure. This change will
 * allow to use
...
#define MMU2_RAMMING_SEQUENCE                                                                      \
    {1.0, 1000}, {1.0, 1500}, {2.0, 2000}, {1.5, 3000}, {2.5, 4000}, {-15.0, 5000}, {-14.0, 1200}, \
        {-6.0, 600}, {10.0, 700}, {-10.0, 400}, {                                                  \
        -50.0, 2000                                                                                \
    }
#endif
